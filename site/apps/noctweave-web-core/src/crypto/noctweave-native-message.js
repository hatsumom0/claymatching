...
export async function encryptNativeTextEnvelope({
  ...
  return encryptNativeMessageEnvelope({
    crypto,
    pqc,
    identity,
    contact,
...
export async function prepareNativeMessageKey({ crypto, conversation }) {
  return nextMessageKey(crypto, conversation.sendChain);
}

export async function encryptNativeMessageEnvelope({
  crypto,
  pqc,
  identity,
  conversation,
  body,
  preparedMessageKey = null,
  kemCiphertext = null,
  sentAt = swiftISODate()
}) {
  const ownSigning = deserializeKeypair(identity.signing);
  const prepared = preparedMessageKey ?? await nextMessageKey(crypto, conversation.sendChain);
  const plaintext = encodePaddedBody(body, (length) => crypto.randomBytes(length));
...
export async function decryptNativeEnvelope({ crypto, pqc, identity, contact, conversation, envelope }) {
  const decrypted = await decryptNativeMessageEnvelope({ crypto, pqc, identity, contact, conversation, envelope });
...
export async function decryptNativeMessageEnvelope({ crypto, pqc, identity, contact, conversation, envelope }) {
  ...
  return {
    body: decodePaddedBody(plaintext),
    messageKey: key
  };
}
...
function encodePaddedBody(body, randomBytes) {
...
function decodePaddedBody(data) {
  let bodyData = data;
  ...
  return JSON.parse(decoder.decode(bodyData));
}
